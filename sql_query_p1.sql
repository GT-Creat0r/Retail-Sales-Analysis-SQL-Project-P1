...
-- Q.3 Write a SQL query to calculate the total sales (total_sale) for each category.
	SELECT category ,  -- this is the column 1
			SUM(total_sale) AS net_sale,
			COUNT(*) AS total_orders
	FROM retail_sales
	GROUP BY 1;  -- 1 represents first column in the SELECT list , it is equivalent to GROUP BY category;
...
-- Q.8 Write a SQL query to find the top 5 customers based on the highest total sales 
	SELECT customer_id,
		...
		RANK() OVER(ORDER BY SUM(total_sale) DESC ) AS rank -- just for showing the rank of rows (not compoulsary in this scenario).
	FROM retail_sales
